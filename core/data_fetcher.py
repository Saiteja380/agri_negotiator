...
# Load our secure API keys from the .env file
load_dotenv()
...
def get_mandi_price(state: str, commodity: str) -> float:
    ...
    # We removed the broken URL sort and added &limit=100 to grab a healthy batch
    ...
    try:
        ...
        if len(records) > 0:
            # Bypass: We sort the records right here in Python based on Arrival_Date (DD/MM/YYYY)
            records.sort(key=lambda x: datetime.strptime(x.get('Arrival_Date', '01/01/2000'), "%d/%m/%Y"), reverse=True)
            
            # Return the Modal_Price of the newest record at index 0
...
if __name__ == "__main__":
    # Test run: Negotiating 10 Tons of Potato from Agra to Lucknow
    final_contract = run_negotiation(
        commodity="Potato",
        state="Uttar Pradesh",
        start_lat=27.17,   # Agra Lat
        start_lon=78.00,   # Agra Lon
        end_lat=26.84,     # Lucknow Lat
        end_lon=80.94,     # Lucknow Lon
        quantity_tons=10.0
    )
    
    print("\n=======================================")
    print("        FINAL NEGOTIATED CONTRACT        ")
    print("=======================================")
